...
def data_parems(result, proxies):
    ...
    link_message["cove"] = (
        result["thumbnails"][0].get("url") if result["thumbnails"] else ""
    )
    link_message["height"] = (
        result["thumbnails"][0].get("height") if result["thumbnails"] else ""
    )
    link_message["width"] = (
        result["thumbnails"][0].get("width") if result["thumbnails"] else ""
    )
