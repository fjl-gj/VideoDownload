...
class MyDialog(QDialog):
    def __init__(self, title, message, parent=None):
        ...
        self.button2 = PyPushButton(
            "Yes,And Delete Local File", 8, "#000000", "#70B4FD", "#4A5A71", "#037aff"
        )
        self.button2.setObjectName("2")
        self.button3 = PyPushButton(
            "Cancel", 8, "#000000", "#70B4FD", "#4A5A71", "#037aff"
        )
...
class PyDownloadWidget(QWidget):
    ...
    def __init__(
        ...
        self.index = i
        self.values = (
            formats.get("down_percentage") if formats.get("down_percentage") else 0
        )
        self.pause_status = 0
        ...
        self.file_type = (
            formats.get("file_type") if formats.get("file_type") else formats.get("ext")
        )
        ...
        self.size = (
            formats.get("size") if formats.get("size") else formats.get("filesize")
        )
        self.down_file_size = (
            formats.get("down_file_size") if formats.get("down_file_size") else 0
        )
        self.chunk_size = 1024
        self.file_name = (
            "VD" + self.format + replace_other_char(self.title) + self.format_id
        )
        if self.formats.get("file_path_name"):
            self.file_path_name = self.formats.get("file_path_name")
        else:
            self.file_path_name = (
                globals_var.DOWNLOAD_DIRECTORY.replace("\\", "\\\\")
                + f"\\\\{self.file_name}.{self.file_type}"
            )
        self.file_path_info_json = (
            globals_var.DOWNLOAD_DIRECTORY + f"\\{self.file_name}.info.json"
        )
        ...
        self.load_open_folder = PyIconButton(
            Functions.set_svg_icon("icon_folder_open.svg")
        )
    ...
    def pause_status_action(self):
        ...
        if not self.pause_status:
            ...
            self.load_pause.close()
            self.link_status.setText("PAUSE")
            self.load_resume = PyIconButton(
                Functions.set_svg_icon("icon_downloading.svg")
            )
            self.load_button.addWidget(self.load_resume)
            self.load_resume.clicked.connect(self.resume_status_action)
            if self.index:
    ...
    def resume_status_action(self):
        if self.values != 100 and self.values < 100:
            if not self.resume_status:
                ...
                if self.index:
                    self.download_widget_status.emit(
                        [self.index, self.format_id, "RESUME-DOWNLOAD"]
                    )
    ...
    def download_data(self):
        ...
        url = (
            self.formats.get("down_url")
            if self.formats.get("down_url")
            else self.formats.get("url")
        )
        # logger.info(f'{self.down_file_size}, {self.size}, {url}')

        resp = requests.get(
            url, headers=_headers, proxies=_proxies, stream=True, timeout=15
        )
    ...
    def set_value_num(self, bar):
        ...
        if self.resume_status:
            if self.index:
                self.download_widget_status.emit(
                    [self.index, self.format_id, "Downloading"]
                )
            self.resume_status = 0
        ...
        if bar == 100:
            ...
            if self.index:
                self.download_widget_status.emit(
                    [self.index, self.format_id, "FINISHED"]
                )
            # if os.path.exists(self.file_path_name):
            # self.load_button.addWidget(self.load_open_folder)
    ...
    def check_sql(self):
        ...
        if results:
            for result in results:
                ...
                download_widget = PyDownloadWidget(
                    self.ui,
                    basics_info,
                    result,
                    sql_id,
                ...
                download_widget.load_delete.clicked.connect(
                    lambda: self.remove_list_item(self.download_list)
                )
